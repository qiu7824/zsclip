...
pub(super) unsafe fn settings_create_hotkey_system_controls(
    ...
    let (_, note_height) = b.label_auto(st, "说明：通过注册表 DisabledHotkeys 屏蔽或恢复 Win+V。修改后通常需要重启资源管理器或重新登录。", sec1.left(), sec1.row_y(0), sec1.full_w(), note_h);
    let action_y = sec1.row_y(1).max(sec1.row_y(0) + note_height + small_gap);
