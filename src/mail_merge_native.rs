  ...
  Write-Output ('row_count=' + [Math]::Max(0, $lastRow - $headerRow))
    ...
    let out = ps_run(
        ...
        &[
            ...
            data_row.max(1).to_string(),
        ],
    )?;
    if out.trim().is_empty() {
        return Err(format!(
            "解析 Excel 数据失败: PowerShell 没有返回任何内容 [{}]",
            excel_path
        ));
    }
...
unsafe fn create_ctrl(
    ...
    let caption = if matches!(class, "STATIC" | "BUTTON") {
...
fn button_kind(id: isize) -> SettingsComponentKind {
    match id {
        IDC_LOAD | IDC_INSERT_FIELD | IDC_INSERT_INLINE | IDC_INSERT_TABLE => {
            SettingsComponentKind::AccentButton
        }
...
unsafe fn refresh_mode_ui(hwnd: HWND) {
    ...
    set_text(
        hwnd,
        IDC_INSERT_FIELD,
        if fill { "粘贴字段" } else { "插入字段" },
    );
    set_text(
        hwnd,
        IDC_INSERT_INLINE,
        if fill { "粘贴一行" } else { "插入一行" },
    );
    set_text(
        hwnd,
        IDC_INSERT_TABLE,
        if fill { "填表格" } else { "插入表格" },
    );
    for id in [
        IDC_DATA_ROW,
        IDC_PREV_ROW,
...
unsafe fn paste_text_like_main(
    hwnd: HWND,
    st: &mut MailMergeState,
    text: &str,
) -> Result<(), String> {
...
unsafe fn paste_merge_field_like_main(
    hwnd: HWND,
    st: &mut MailMergeState,
    field: &str,
) -> Result<(), String> {
...
unsafe fn reload_excel(hwnd: HWND, st: &mut MailMergeState) {
    ...
    if st.excel_path == excel
        && st.active_sheet == sheet
        ...
        && st.data_row != data_row
    {
        match ps_fetch_excel_row_values(&excel, &sheet, header_row, data_row) {
            Ok((values, row_count)) => {
                ...
                set_status(
                    hwnd,
                    &format!("已切换到第 {} 行，共 {} 行数据", st.data_row, st.row_count),
                );
            }
...
unsafe fn run_primary_action_inner(hwnd: HWND, st: &mut MailMergeState, throttle: bool) {
    ...
    if let Some(field) = selected_field(hwnd) {
        ...
        let res = if is_fill_mode(hwnd) {
            paste_text_like_main(hwnd, st, &value)
        } else {
            paste_merge_field_like_main(hwnd, st, &field)
        };
        match res {
            Ok(_) => set_status(
                hwnd,
                if is_fill_mode(hwnd) {
                    "已粘贴当前字段"
                } else {
                    "已插入字段"
                },
...
unsafe extern "system" fn wnd_proc(
    ...
    match msg {
        WM_CREATE => {
            ...
            let args = if !cs.lpCreateParams.is_null() {
                Box::from_raw(cs.lpCreateParams as *mut CreateArgs)
            } else {
                Box::new(CreateArgs {
            ...
            create_ctrl(
                "LISTBOX",
                "",
                LBS_NOTIFY as u32 | WS_VSCROLL | WS_TABSTOP,
                WS_EX_CLIENTEDGE,
                16,
                228,
                654,
            ...
            create_ctrl(
                "STATIC",
                "请选择 Excel 文件（xlsx/xlsm/xls/csv）",
                0,
                0,
                16,
                492,
            ...
            match id {
                ...
                IDC_GUESS => {
                    ...
                    if excel.trim().is_empty() {
                        ...
                        match ps_guess_header_row(&excel, &current_combo_text(hwnd)) {
                            Ok(row) => {
                                set_text(hwnd, IDC_HEADER_ROW, &row.to_string());
                                reload_excel(hwnd, st);
                            }
                            Err(e) => set_status(hwnd, &e),
                        }
                ...
                IDC_PREV_ROW => {
                    set_text(
                        hwnd,
                        IDC_DATA_ROW,
                        &(current_data_row(hwnd) - 1).max(1).to_string(),
                    );
                    reload_excel(hwnd, st);
                }
                IDC_NEXT_ROW => {
                    let next = if st.row_count > 0 {
                        (current_data_row(hwnd) + 1).min(st.row_count as i32)
                    } else {
                        current_data_row(hwnd) + 1
                    };
                    set_text(hwnd, IDC_DATA_ROW, &next.to_string());
                    reload_excel(hwnd, st);
                }
                IDC_APPLY_ROW => reload_excel(hwnd, st),
                IDC_MODE_MERGE | IDC_MODE_FILL => {
                    refresh_mode_ui(hwnd);
                    if is_fill_mode(hwnd) {
                        reload_excel(hwnd, st);
                    }
                ...
                IDC_COPY_FIELDS => {
                    let mut cb = match Clipboard::new() {
                        Ok(v) => v,
                        Err(e) => {
                            set_status(hwnd, &format!("打开剪贴板失败: {e}"));
                            return 0;
                        }
                    };
                    match cb.set_text(st.headers.join("\r\n")) {
                        Ok(_) => set_status(hwnd, "已复制字段名"),
                        Err(e) => set_status(hwnd, &format!("复制字段失败: {e}")),
                    }
                }
                IDC_OPEN_WORD => match ps_word_open() {
                    Ok(_) => set_status(hwnd, "已打开 Word"),
                    Err(e) => set_status(hwnd, &e),
                },
                IDC_INSERT_FIELD => run_primary_action(hwnd, st),
                IDC_INSERT_INLINE => {
                    let res = if is_fill_mode(hwnd) {
                        paste_text_like_main(hwnd, st, &st.values.join(" "))
                    } else {
